...
class Login extends ChangeNotifier{
  ...
  GlobalKey<ScaffoldState> scaffoldKeyCadastar = GlobalKey<ScaffoldState>(debugLabel: '_scaffoldKeySinup');
  ...
  Future<String> salvarImagem()async {
    ...
    final firebaseStorageRef = FirebaseStorage.instance.ref().child("usuarios/"+imagem.path.split('/').last);
      ...
      if(response.statusCode == 200){
        ...
        _carregarUserAtual(
          usuario: User(
            ...
            expiresIn: DateTime.now().add(Duration(seconds: int.parse(response.data['expiresIn']))),
  ...
  Future<void> postCadastrarUser({
    ...
    try{
      ...
      if(response.statusCode == 200){
        ...
        if(response.statusCode == 200){
          Map<String, dynamic> data = new Map<String, dynamic>.from(response1.data);
  ...
  void retornarMensagemCadastrar({
    ...
    scaffoldKeyCadastar.currentState!.showSnackBar(
        SnackBar(
          content: Text(
            mensagem,
            textAlign: TextAlign.center,
  ...
  Future<void> _carregarUserAtual({ required User usuario}) async {
    ...
    if(prefs.getString('idToken') != null){
      ...
      user = User(
        expiresIn: DateFormat("yyyy-MM-dd hh:mm:ss").parse(prefs.getString('expiresIn')??""),
